...
import { login as api_login, logout as api_logout, verify_2fa_login, is_logged_in as check_is_logged_in, get_permissions } from '@/api/auth'
...
// 登录（第一步）
async function login(username: string, password: string): Promise<{ success: boolean; need_2fa: boolean }> {
    loading.value = true
    error.value = null
...
// 检查权限
function has_permission(type: 'can_add' | 'can_edit' | 'can_delete' | 'can_toggle_visibility'): boolean {
