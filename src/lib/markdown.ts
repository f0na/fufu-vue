  ...
  for (const block of math_blocks) {
    try {
      const rendered = block.display
        ? `<div class="katex-block my-4 overflow-x-auto text-center">${katex.renderToString(block.raw, { displayMode: true, throwOnError: false })}</div>`
...
export function setup_copy_handler() {
  if (typeof window !== 'undefined') {
    (window as any).copyCode = function (btn: HTMLButtonElement, _id: string) {
