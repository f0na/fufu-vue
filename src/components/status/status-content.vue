...
const data_stats = computed(() => {
  ...
  return [
    ...
    { key: 'friends', label: '友人帐', value: s.friends, icon: 'lucide:users' },
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              ...
              <p class="text-sm font-medium text-foreground flex items-center gap-1.5 mt-0.5">
                <span
                  ...
                  :class="status_data.api.status === 'ok' ? 'bg-green-500' : 'bg-red-500'"
                />
            ...
            <div>
              ...
              <p class="text-sm font-medium text-foreground mt-0.5">
                {{ status_data.api.version }}
              </p>
            </div>
            <div>
              <p class="text-xs text-muted-foreground">运行时间</p>
              <p class="text-sm font-medium text-foreground mt-0.5">
                {{ fmt_uptime(status_data.api.uptime) }}
              </p>
            </div>
            <div>
              <p class="text-xs text-muted-foreground">数据库延迟</p>
              <p class="text-sm font-medium text-foreground mt-0.5">
                D1 {{ status_data.api.d1.latency_ms }}ms / KV {{ status_data.api.kv.latency_ms }}ms
              </p>
            </div>
          </div>
        </CardContent>
