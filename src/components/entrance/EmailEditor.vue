...
const editor = useEditor({
  ...
  editorProps: {
    attributes: {
      class:
        'w-full min-h-[120px] max-h-[200px] px-4 py-2 text-base rounded-lg border border-gray-300 focus:border-rose-400 focus:ring-2 focus:ring-rose-400/20 focus:outline-none transition-all overflow-y-auto bg-white max-w-none',
    },
  },
})
...
<template>
  <div
    class="fixed inset-0 flex items-center justify-center bg-black/50 z-[100]"
    @click.self="
      emit('close')
      show_image_menu = false
    "
  >
    <div
      class="flex flex-col gap-4 p-6 rounded-2xl backdrop-blur-md bg-white/95 border border-white/30 w-[480px] max-w-[90vw] max-h-[85vh] overflow-hidden"
    >
      <!-- 标题 -->
      ...
      <div class="flex items-center gap-1 p-2 rounded-lg bg-gray-100 border border-gray-200">
        ...
        <div class="relative">
          ...
          <!-- 图片选择菜单 -->
          <div
            v-if="show_image_menu"
            class="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10 min-w-32"
            @click.stop
          >
            <button
              @click="upload_local_image"
      ...
      <div v-if="attachments.length > 0" class="flex flex-col gap-2 max-h-24 overflow-y-auto">
        <div
          ...
          :key="file.name + index"
          class="flex items-center justify-between px-3 py-2 rounded-lg bg-gray-100"
        >
          ...
          <button
            ...
            class="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-200"
          >
      ...
      <div class="flex gap-3 pt-2">
        <label
          class="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors cursor-pointer"
        >
        ...
        <button
          @click="handle_send"
          class="flex-1 px-5 py-2.5 rounded-lg bg-gradient-to-r from-[var(--c-primary)] to-[var(--c-secondary)] text-white font-medium transition-all hover:shadow-lg hover:shadow-[var(--c-primary)]/20"
        >
          发送邮件
        </button>
      </div>
