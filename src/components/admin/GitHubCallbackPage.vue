...
onMounted(async () => {
    ...
    // 验证state
    const saved_state = sessionStorage.getItem('github_state')
    if (saved_state !== state) {
        error.value = 'state验证失败，请重新登录'
    ...
    <div class="p-6 rounded-xl bg-white border border-[var(--c-border)] shadow-sm max-w-md mx-auto">
        <div class="text-center">
            ...
            <template v-if="loading">
                <div class="i-lucide-loader-2 w-12 h-12 text-[var(--c-primary)] animate-spin mx-auto mb-4" />
