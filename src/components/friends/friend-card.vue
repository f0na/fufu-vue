...
const show_favicon_api = computed(() => friend_info.value.favicon_api_url && favicon_api_loaded.value && !favicon_loaded.value)
const show_initial = computed(() => friend_info.value.site_initial && /^[A-Za-z0-9]$/.test(friend_info.value.site_initial))
...
watch(() => friend_info.value.primary_avatar, (val) => {
  if (val && !avatar_loaded.value) {
    preload_image(val, () => { avatar_loaded.value = true })
  }
}, { immediate: true })

watch(() => friend_info.value.favicon_url, (val) => {
  if (val && !favicon_loaded.value) {
    preload_image(val, () => { favicon_loaded.value = true })
  }
}, { immediate: true })

watch(() => [friend_info.value.favicon_api_url, favicon_loaded.value, favicon_api_loaded.value], () => {
  if (friend_info.value.favicon_api_url && !favicon_api_loaded.value && !favicon_loaded.value) {
    preload_image(friend_info.value.favicon_api_url, () => { favicon_api_loaded.value = true })
  }
}, { immediate: true })
</script>
