        ...
        <div v-if="show_apply_modal" class="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            <div class="relative w-full max-w-md mx-4 bg-white rounded-xl shadow-xl overflow-hidden">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-[var(--c-border)]">
                    ...
                    <button @click="close_apply_modal"
                ...
                <div class="p-6 flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input v-model="apply_form.name" type="text" placeholder="请输入站点名称"
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input v-model="apply_form.url" type="url" placeholder="https://example.com"
                            class="w-full px-3 py-2 text-sm rounded-lg border border-[var(--c-border)] bg-white focus:border-[var(--c-primary)] focus:ring-2 focus:ring-[var(--c-primary)]/20 focus:outline-none transition-all"
                            @input="on_url_change" />
                    </div>

                    <!-- 图标预览 -->
                    <div v-if="show_icon_preview"
                        ...
                        <div
                            ...
                            <img :src="preview_icon_url" alt="网站图标" class="w-6 h-6 object-contain"
                                @error="on_icon_error" />
                        </div>
                    ...
                    <!-- 网站描述 -->
                    <div class="flex flex-col gap-1.5">
                        <label class="text-sm text-slate-600">网站描述 <span class="text-slate-400">(可选)</span></label>
                        <textarea v-model="apply_form.description" placeholder="简单介绍一下你的网站" rows="3"
                ...
                <!-- 操作按钮 -->
                <div class="flex items-center justify-end gap-2 px-6 py-4 border-t border-[var(--c-border)]">
                    <button @click="close_apply_modal"
                        class="px-4 py-2 text-sm rounded-lg text-slate-500 hover:bg-slate-100 transition-colors">
                        取消
                    </button>
                    <button @click="submit_apply" :disabled="apply_loading"
