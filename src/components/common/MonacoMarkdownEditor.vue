...
function register_custom_actions() {
  ...
  editor_instance.addAction({
    ...
    label: '在上方插入新行',
    keybindings: [
      monaco_instance.KeyMod.CtrlCmd | monaco_instance.KeyMod.Shift | monaco_instance.KeyCode.Enter,
    ],
    run: (ed) => {
      const position = ed.getPosition()
      if (!position) return
...
function insert_text(text: string) {
  ...
  editor_instance.executeEdits('', [
    {
      range: new monaco_instance.Range(
        position.lineNumber,
        position.column,
        position.lineNumber,
        position.column,
