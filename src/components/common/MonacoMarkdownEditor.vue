...
async function init_editor() {
    ...
    try {
        ...
        // 监听焦点事件
        editor_instance.onDidFocusEditorWidget(() => {
            emits('focus')
        })

        editor_instance.onDidBlurEditorWidget(() => {
            emits('blur')
        })
...
function update_editor_height() {
    if (!editor_instance || !editor_container.value) return

    const content_height = editor_instance.getContentHeight()
...
function register_custom_actions() {
    if (!editor_instance || !monaco_instance) return

    // Ctrl/Cmd + Enter: 在下方插入新行
    editor_instance.addAction({
        id: 'insert-line-below',
        label: '在下方插入新行',
        keybindings: [monaco_instance.KeyMod.CtrlCmd | monaco_instance.KeyCode.Enter],
        run: (ed) => {
            ...
            ed.executeEdits('', [{
                range: new monaco_instance!.Range(newLine, 1, newLine, 1),
                text: '\n',
            }])
    ...
    editor_instance.addAction({
        ...
        label: '在上方插入新行',
        keybindings: [monaco_instance.KeyMod.CtrlCmd | monaco_instance.KeyMod.Shift | monaco_instance.KeyCode.Enter],
        run: (ed) => {
            const position = ed.getPosition()
            if (!position) return

            ed.executeEdits('', [{
                range: new monaco_instance!.Range(position.lineNumber, 1, position.lineNumber, 1),
                text: '\n',
            }])
    ...
    editor_instance.addAction({
        id: 'format-bold',
        label: '加粗',
        keybindings: [monaco_instance.KeyMod.CtrlCmd | monaco_instance.KeyCode.KeyB],
    ...
    editor_instance.addAction({
        id: 'format-italic',
        label: '斜体',
        keybindings: [monaco_instance.KeyMod.CtrlCmd | monaco_instance.KeyCode.KeyI],
        run: (ed) => wrap_selection(ed, '*', '*'),
    })
    ...
    editor_instance.addAction({
        id: 'insert-link',
        label: '插入链接',
        keybindings: [monaco_instance.KeyMod.CtrlCmd | monaco_instance.KeyCode.KeyK],
    ...
    editor_instance.addAction({
        id: 'format-code',
        label: '行内代码',
        keybindings: [monaco_instance.KeyMod.CtrlCmd | monaco_instance.KeyCode.Backquote],
        run: (ed) => wrap_selection(ed, '`', '`'),
    })
...
function setup_vim_keybindings() {
    ...
    // Escape: 返回普通模式
    editor_instance.addCommand(monaco_instance.KeyCode.Escape, () => {
    ...
    editor_instance.addAction({
        id: 'vim-insert-mode',
        label: 'Vim: 插入模式',
        keybindings: [monaco_instance.KeyCode.KeyI],
        run: () => {
...
function wrap_selection(
    ...
    if (selected_text) {
        ed.setSelection(new monaco_instance.Selection(
            start.lineNumber,
            start.column,
            end.lineNumber,
...
function insert_text(text: string) {
    ...
    const position = editor_instance.getPosition()
    if (!position) return

    editor_instance.executeEdits('', [{
        range: new monaco_instance.Range(position.lineNumber, position.column, position.lineNumber, position.column),
        text,
    }])
...
function insert_heading(level: number) {
    ...
    const model = editor_instance.getModel()
    if (!model) return
    ...
    if (heading_match && heading_match[1]) {
        ...
        editor_instance.executeEdits('', [{
            range: new monaco_instance.Range(position.lineNumber, 1, position.lineNumber, line_content.length + 1),
            text: new_content,
        }])
    } else {
        // 在行首插入标题标记
...
function wrap_selection_external(prefix: string, suffix: string) {
    if (editor_instance) {
        wrap_selection(editor_instance, prefix, suffix)
    }
}
...
watch(content, (new_value) => {
    if (editor_instance) {
        const current_value = editor_instance.getValue()
        if (current_value !== new_value) {
            editor_instance.setValue(new_value)
...
watch(() => props.disabled, (disabled) => {
    if (editor_instance) {
        editor_instance.updateOptions({ readOnly: disabled })
    }
})
...
<template>
    <div class="monaco-editor-wrapper">
        <div ref="editor_container" class="monaco-markdown-editor" />

        <!-- Vim 模式状态栏 -->
        <div v-if="vimMode && is_ready" class="vim-status-bar" :class="{
