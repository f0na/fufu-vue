...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div class="p-3 rounded-xl bg-white border border-[var(--c-border)] shadow-sm">
      ...
      <ExpandingInput v-model="form_data.title" placeholder="文章标题" />
    </div>

    <!-- Slug -->
    <div class="p-3 rounded-xl bg-white border border-[var(--c-border)] shadow-sm">
      <label class="text-xs font-medium text-slate-500 mb-1 block">Slug</label>
      <ExpandingInput v-model="form_data.slug" placeholder="url-slug" :disabled="slug_generating" />
      <button
        @click="handle_generate_slug"
        :disabled="slug_generating || !form_data.title"
    ...
    <div class="p-3 rounded-xl bg-white border border-[var(--c-border)] shadow-sm">
      ...
      <div v-if="cover_display_url" class="relative group">
        <div
          class="relative w-full aspect-video rounded-lg overflow-hidden border border-slate-200 cursor-pointer"
          @click="trigger_cover_upload"
        >
      ...
      <div v-else class="relative group cursor-pointer" @click="trigger_cover_upload">
        <div
          class="w-full aspect-video rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center gap-1 text-slate-400 hover:border-[var(--c-primary)] hover:bg-[var(--c-primary-bg)]/30 transition-colors"
        >
      ...
      <!-- URL 输入框 -->
      <ExpandingInput v-model="form_data.cover" placeholder="或输入图片URL" class="mt-2" />
      <input
        ref="cover_input_ref"
        type="file"
        accept="image/*"
        class="hidden"
        @change="handle_cover_select"
      />
    </div>
    ...
    <div class="p-3 rounded-xl bg-white border border-[var(--c-border)] shadow-sm">
      ...
      <ExpandingInput v-model="form_data.category" placeholder="分类名称" list="category-options" />
    ...
    <div class="p-3 rounded-xl bg-white border border-[var(--c-border)] shadow-sm">
      <div class="flex items-center justify-between">
        ...
        <button
          type="button"
          @click="form_data.top = !form_data.top"
    ...
    <div class="p-3 rounded-xl bg-white border border-[var(--c-border)] shadow-sm">
      <div class="flex items-center justify-between">
        ...
        <button
          type="button"
          @click="form_data.comment_allowed = !form_data.comment_allowed"
          class="relative w-10 h-5 rounded-full transition-colors"
