...
const emit = defineEmits<{
  ...
  (e: 'update-size', id: string, width: number, height: number): void
  (e: 'rotate', id: string, rotation: number): void
}>()
...
const display_size = computed(() => {
  if (!loaded.value) {
    ...
    return {
      width: props.photo.width || default_width,
      height: props.photo.height || default_height,
    }
  }
...
const photo_style = computed(() => ({
  // 默认 transform-origin 是左上角
  // translate 移动左上角位置，rotate 以左上角旋转
  // 用户拖动旋转手柄时，我们计算相对于视觉中心的旋转角度
  transform: `translate(${current_x.value}px, ${current_y.value}px) rotate(${current_rotation.value}deg)`,
  zIndex: props.z_index,
}))

// 处理图片 URL，添加自动校正 EXIF 方向参数
const processed_src = computed(() => add_auto_orient(props.photo.src))

// 监听外部旋转变化（拖动和旋转过程中都跳过）
watch(
  () => props.photo.rotation,
  (new_rotation) => {
    // 拖动或旋转过程中不同步外部值，避免干扰用户操作
    if (!is_dragging.value && !is_rotating.value) {
...
function handle_rotate_start(e: MouseEvent) {
  ...
  rotate_start_angle =
    get_angle(e.clientX, e.clientY, rotate_center_x, rotate_center_y) - current_rotation.value
...
function handle_rotate_touch_start(e: TouchEvent) {
  ...
  const touch = e.touches[0]!
  rotate_start_angle =
    get_angle(touch.clientX, touch.clientY, rotate_center_x, rotate_center_y) -
...
<template>
  <div
    ...
    :class="{
      'shadow-xl': is_dragging,
      'ring-4 ring-red-500': delete_mode,
    }"
    :style="photo_style"
