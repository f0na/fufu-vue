    ...
    try {
      ...
      const all_states = g.photos.map((photo, index) => {
        ...
        return existing || {
          id: photo,
          x: 30 + Math.random() * Math.max(100, viewport_width - photo_width - 60),
          y: 80 + Math.random() * Math.max(100, viewport_height - 300) + index * (mobile.value ? 15 : 25),
          rotation: random_rotation(),
          z_index: index,
        }
      })
...
watch(() => props.gallery_id, (gallery_id) => {
...
function handle_wheel(e: WheelEvent) {
  if (lightbox_photo.value || show_comments.value || dragging_photo.value || rotating_photo.value || canvas_dragging.value) return
...
function handle_mouse_move(e: MouseEvent) {
  ...
  if (rotating_photo.value) {
    const current_angle = Math.atan2(
      e.clientY - rotate_center_y,
      e.clientX - rotate_center_x
    ) * (180 / Math.PI)

    const new_rotation = initial_rotation.value + (current_angle - rotate_start_angle)

    photo_states.value = photo_states.value.map(state =>
      state.id === rotating_photo.value
...
function handle_rotate_start(e: MouseEvent, photo_id: string, state: PhotoState) {
  ...
  rotate_start_angle.value = Math.atan2(e.clientY - center_y, e.clientX - center_x) * (180 / Math.PI)
}
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Icon icon="lucide:rotate-cw" class="text-slate-500" :style="{ width: mobile ? '16px' : '14px', height: mobile ? '16px' : '14px' }" />
          </div>
        </div>
      </div>
    </div>
