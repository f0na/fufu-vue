...
const filtered_groups = computed(() =>
  props.weekday_groups.filter(g => selected_weekday.value === null || g.weekday === selected_weekday.value)
)
</script>
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div v-if="is_loading" class="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
      <div v-for="i in 6" :key="i" class="break-inside-avoid overflow-hidden rounded-xl border border-border">
