    ...
    : [
        {
          ...
          records: grouped_records.value[selected_status.value] || [],
        },
      ]
);
</script>
...
<template>
  <div v-if="records.length > 0" class="flex flex-col gap-6">
    ...
    <!-- 记录列表 -->
    <div
      v-for="{ status, records: group_records } in display_groups"
      :key="status"
    ...
    >
      ...
      >
        <div
          ...
          :class="
            cn(
              'break-inside-avoid overflow-hidden cursor-pointer group rounded-xl border border-border'
            )
          "
