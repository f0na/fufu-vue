...
const can_submit = computed(() => {
    ...
    if (show_guest_form.value && !guest_name.value.trim()) return false
    return true
})
...
<template>
    <div class="flex flex-col gap-3">
        ...
        <!-- 游客信息表单 -->
        <div v-if="show_guest_form" class="flex flex-col gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200">
            ...
            <div class="flex flex-wrap gap-2">
                <input
                    ...
                    type="text"
                    placeholder="名称 *"
                    class="flex-1 min-w-[120px] px-3 py-2 text-sm rounded-lg border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-[var(--c-primary)]/20 focus:border-[var(--c-primary)]"
                    maxlength="20"
                />
                <input
                    v-model="guest_email"
                    type="email"
                    placeholder="邮箱（可选）"
                    class="flex-1 min-w-[150px] px-3 py-2 text-sm rounded-lg border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-[var(--c-primary)]/20 focus:border-[var(--c-primary)]"
                />
            </div>
            <input
