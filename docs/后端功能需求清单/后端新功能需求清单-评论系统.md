...
**说明**：支持相册和番剧的评论功能，两级评论结构，登录用户和游客均可评论。
...
interface Comment {
  ...
  content: string               // 评论内容（Markdown格式，已过滤敏感词，1-500字符）
  is_markdown: boolean          // 是否为 Markdown 格式，默认 false（普通文本）
...
| `is_markdown` | boolean | 否 | 是否为 Markdown 格式，默认 false |
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/v1/admin/comments` | GET | 获取所有评论列表 |
| `/api/v1/admin/comments/:id` | DELETE | 删除评论 |
| `/api/v1/admin/comments/:id/status` | PATCH | 修改评论状态 |
| `/api/v1/admin/sensitive-words` | GET | 获取敏感词列表 |
| `/api/v1/admin/sensitive-words` | POST | 添加敏感词 |
...
{
  "word": "敏感词",
  "level": "filter"  // filter: 替换为*** | hide: 自动隐藏评论
}
```
...
1. 评论发布时自动检测敏感词
2. 敏感词替换为 `***`
3. 替换后的内容存入 `content`，原始内容存入 `raw_content`
4. 严重敏感词（level=hide）直接设置 `status: hidden`
...
| 组件路径 | 说明 |
|---------|------|
| `src/components/comment/CommentSection.vue` | 评论区域主组件 |
| `src/components/comment/CommentItem.vue` | 单条评论组件 |
| `src/components/comment/CommentForm.vue` | 评论表单组件 |
| `src/components/comment/GuestInfoForm.vue` | 游客信息表单 |

### 评论展示位置

| 页面 | 路由 | 说明 |
|------|------|------|
| 相册空间页 | `/gallery/:id` | 相册详情页展示评论 |
| 番剧详情页 | 待实现 | 番剧详情页展示评论 |
