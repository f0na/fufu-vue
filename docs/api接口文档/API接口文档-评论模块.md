...
interface Comment {
  ...
  guest_email: string | null    // 游客邮箱（必须，验证邮箱真实性）
...
| `guest_email` | string | 游客必填 | 游客邮箱（需为有效邮箱格式） |
| `guest_avatar` | string | 否 | 游客头像URL |
| `content` | string | 是 | 评论内容（1-500字符） |
| `markdown` | boolean | 否 | 是否为 Markdown 格式，默认 false |
...
GET /api/v1/admin/sensitive-words?page=1&per_page=20
GET /api/v1/admin/sensitive-words?level=hide
Authorization: Bearer <token>
```
...
| `level` | string | - | 按级别筛选：filter / hide |
